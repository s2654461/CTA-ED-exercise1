...
# 定义种族相关词汇
edbf_term_counts_race$raceword <- as.integer(grepl("race|racial|racist|racism|ethnic|diversity|multicultural|discrimination|prejudice|minority|black|asian|indigenous|immigration|colonial|colonialism", 
...
ggplot(edbf_counts_race, aes(year, sum_race / year_total, group=1)) +
  geom_line() +
  geom_vline(xintercept = 2020, col="red", linetype="dashed") +  # 2020年标记线
  xlab("Year") +
...
# 按类型分析趋势
edbf_counts_race_genre <- edbf_term_counts_race %>%
  group_by(year, genre) %>%
  mutate(year_total = sum(n)) %>%
  filter(raceword==1) %>%
  summarise(sum_race = sum(n),
...
# 绘制不同类型的对比图
ggplot(edbf_counts_race_genre, 
       aes(year, sum_race / year_total, 
           group=genre, color=genre)) +
  geom_line() +
  geom_vline(xintercept = 2018, col="red", linetype="dashed") +  # 2018年标记线
  xlab("Year") +
  ylab("% race-related words") +
  labs(color = "Genre") +  # 添加图例标题
  scale_y_continuous(labels = scales::percent_format(),
                     expand = c(0, 0), limits = c(0, NA)) +
  theme_tufte(base_family = "Helvetica")
```
